...
#include <filesystem>
#include "../func/folder_packer/zip_to_cfup.hpp"
...
public:
    ...
    const char* description() const override { 
        return "Convert zip to .cfup archive"; 
    }
    ...
    int run(CommandContext& ctx) override {
        ...
        Logger::StartTimer("Zip packing");
        
        if (!convert_zip_to_cfup(file, packedFilePath.string())) {
            Logger::Log(LOG_ERROR, "Failed to convert zip file");
            return 1;
        }
        
        Logger::EndTimer("Zip packing", LOG_INFO);
